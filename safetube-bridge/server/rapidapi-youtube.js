  ...
  for (let attempt = 0; attempt <= RAPIDAPI_429_MAX_RETRIES; attempt++) {
    ...
    if (res.status === 429) {
      if (attempt >= RAPIDAPI_429_MAX_RETRIES) {
        throw new Error(`RapidAPI ${label} HTTP 429 after ${attempt + 1} attempts (rate limited)`);
      }
      const delayMs = RAPIDAPI_429_BASE_MS * 2 ** attempt;
      console.warn(`[rapidapi] ${label} HTTP 429 — retry ${attempt + 1}/${RAPIDAPI_429_MAX_RETRIES} in ${delayMs}ms`);
      await sleep(delayMs);
      continue;
    }
    return res;
...
function requireApiKey() {
  if (!RAPIDAPI_KEY) throw new Error('RAPIDAPI_KEY is not set on the Media Bridge');
}
...
function pickVideoQualityId(qualities) {
  ...
  const ranked = (capped.length > 0 ? capped : pool).sort(
    (a, b) => parseHeight(b.quality) - parseHeight(a.quality)
  );
...
async function getAvailableQualities(videoId) {
  ...
  if (res.status >= 400) throw new Error(`RapidAPI get_available_quality HTTP ${res.status}`);
...
async function requestDownloadMeta(videoId, qualityId) {
  ...
  for (let attempt = 1; attempt <= FILE_NOT_READY_MAX_ATTEMPTS; attempt++) {
    const res = await rapidApiGet(
      ...
      { params: { quality: qualityId }, headers: rapidHeaders(), timeout: RAPIDAPI_REQUEST_TIMEOUT_MS },
    ...
    if (res.status >= 400) {
      ...
      if (isFileNotReadyHttpResponse(res) && attempt < FILE_NOT_READY_MAX_ATTEMPTS) {
        console.warn(
          `[rapidapi] download_video file not ready video=${videoId} attempt=${attempt}/${FILE_NOT_READY_MAX_ATTEMPTS} — retry in ${FILE_NOT_READY_WAIT_MS}ms`
        );
    ...
    return { file, quality: res.data.quality || null, mime: res.data.mime || 'video/mp4' };
  }
  throw new Error(`RapidAPI download_video file not ready after ${FILE_NOT_READY_MAX_ATTEMPTS} attempts`);
}
...
async function waitForFileReady(fileUrl, { videoId = '' } = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (head.status === 200) {
        console.log(
          `[rapidapi] file ready video=${videoId || '?'} after ${Date.now() - startedAt}ms (attempt ${attempts})`
        );
        return;
      }
      console.warn(
      ...
      console.warn(`[rapidapi] file HEAD error video=${videoId || '?'} attempt=${attempts}: ${err.message}`);
    }
  ...
  console.error(`[rapidapi] ${msg} video=${videoId || '?'}`);
  throw new Error(msg);
}
...
async function resolveVideoDownloadUrl(videoId) {
  ...
  if (!qualityId) {
    qualityId = pickVideoQualityId(await getAvailableQualities(videoId));
  }
  if (!qualityId) throw new Error('No suitable RapidAPI video quality found');
  const meta = await requestDownloadMeta(videoId, qualityId);
  await waitForFileReady(meta.file, { videoId });
  return { url: meta.file, quality: meta.quality, mime: meta.mime };
}
...
async function getVideoInfo(videoId) {
  ...
  if (res.status >= 400) throw new Error(`RapidAPI get-video-info HTTP ${res.status}`);
  return res.data || {};
}

module.exports = { RAPIDAPI_HOST, resolveVideoDownloadUrl, getVideoInfo };
