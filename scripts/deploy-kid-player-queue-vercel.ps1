# One-shot: build, commit kid player next/loop controls, push main, wait for Vercel production.
...
$files = @(
  ...
  'scripts/deploy-kid-player-queue-vercel.ps1'
)
foreach ($f in $files) {
...
if ($pending) {
  git commit -m @"
feat(kid): auto-play next video and loop toggle on player

Add queue controls (next + loop) to CleanPlayer for channel watch and playlist.
Auto-advance when a video ends unless loop is active; loop replays the same video.
"@
...
Write-Host ''
Write-Host 'DONE. Test: https://www.safetube.co.il/kid — צפייה or הפלייליסט שלי' -ForegroundColor Green
