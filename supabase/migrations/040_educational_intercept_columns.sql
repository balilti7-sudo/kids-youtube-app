-- Educational intercept parent settings (schema cache fix).
-- Columns: educational_intercept_enabled (bool), educational_intercept_frequency (text).

ALTER TABLE public.devices
  ADD COLUMN IF NOT EXISTS educational_intercept_enabled BOOLEAN NOT NULL DEFAULT true;

ALTER TABLE public.devices
  ADD COLUMN IF NOT EXISTS educational_intercept_frequency TEXT NOT NULL DEFAULT '3';

-- Migrate legacy column names from migration 039 if present.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'educational_intercepts_enabled'
  ) THEN
    EXECUTE $sql$
      UPDATE public.devices
      SET educational_intercept_enabled = educational_intercepts_enabled
      WHERE educational_intercept_enabled IS DISTINCT FROM educational_intercepts_enabled
    $sql$;
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'devices'
      AND column_name = 'educational_intercept_frequency'
      AND data_type = 'integer'
  ) THEN
    EXECUTE 'ALTER TABLE public.devices ALTER COLUMN educational_intercept_frequency TYPE TEXT USING educational_intercept_frequency::text';
  END IF;
END $$;
...
ALTER TABLE public.devices
  DROP COLUMN IF EXISTS educational_intercepts_enabled;
