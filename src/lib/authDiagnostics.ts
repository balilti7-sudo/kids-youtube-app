/**
 * One-shot global auth-state tracer — DEV only.
 * Never logs emails or token material in production.
 */
...
function shortSession(s: { user?: { id?: string } | null; expires_at?: number | null } | null) {
  if (!s) return null
  return {
